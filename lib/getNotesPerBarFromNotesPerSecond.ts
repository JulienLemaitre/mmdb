 ...
 * @param isTest
 */
export default function getNotesPerBarFromNotesPerSecond({notesPerSecond, beatUnit, bpm, metreNumerator, metreDenominator, isTest = false}:{ notesPerSecond: number, metreNumerator: number, metreDenominator: number, bpm: number, beatUnit: NOTE_VALUE, isTest?: boolean }): number {
  ...
  console.log(`[gNPBFNPS RESULT] notesPerBar: ${notesPerBar} |`, JSON.stringify({ notesPerSecond, metreNumerator, metreDenominator, bpm, beatUnit }))

 ...
 * @param bpm
 */
export default function getNotesPerSecondFromNotesPerBar({ notesPerBar, metreNumerator, metreDenominator, bpm, beatUnit }: { notesPerBar: number, metreNumerator: number, metreDenominator: number, bpm: number, beatUnit: NOTE_VALUE }): number {
  if (!notesPerBar || !metreNumerator || !metreDenominator || !bpm || !beatUnit) {
  ...
  console.log(`[gNPSFNPB RESULT] notesPerSecond: ${notesPerSecond} |`, JSON.stringify({ notesPerBar, metreNumerator, metreDenominator, bpm, beatUnit }))

    ...
    >
      ...
      <div className="text-xs my-3">
        {section?.metronomeMarks?.map((mm: any, idx: number) => {
          if (mm.noMM) {
            return (
              <div key={mm.sectionId || `noMM-${idx}`} className={`italic`}>
                <div>No metronome mark indicated</div>
                {mm.comment && (
                  <div className="text-xs">{`Comment: ${mm.comment}`}</div>
                )}
              </div>
            );
          }
          ...
          let notesPerSecondCollection: NotesPerSecondCollection | null = null;
          try {
          ...
          return (
            <div key={mm.sectionId || `mm-${idx}`} className="space-y-2">
              {notesPerSecondCollection && (
                <div>
              ...
              {mm.comment && (
                <div className="text-xs italic">{`Comment: ${mm.comment}`}</div>
              )}
            </div>
          );
        })}

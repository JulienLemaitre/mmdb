...
describe('reviewDiff rank change detection (per-join)', () => {
  it('emits changed field paths for join ranks that changed and nothing else', () => {
    ...
    const j1 = working.sourceContents.find((j: any) => j.joinId === 'join-1')
    const j2 = working.sourceContents.find((j: any) => j.joinId === 'join-2')
    ...
    const changes = computeChangedChecklistFieldPaths(base as any, working as any)

...
function makeItem(overrides: Partial<RequiredChecklistItem> = {}): RequiredChecklistItem {
  return {
    entityType: 'SECTION',
    entityId: 'sec-1',
...
describe('ChecklistRow visual hints + a11y', () => {
  it('renders Changed badge and announces state', () => {
    ...
    expect(row).toHaveAttribute('aria-label', expect.stringContaining('unchecked'))
    expect(row).toHaveAttribute('aria-label', expect.stringContaining('changed'))
  })
  ...
  it('renders no badges when checked and unchanged', () => {
    ...
    expect(checkbox).toHaveAttribute('aria-label', expect.stringContaining("Tempo indication"))
  })

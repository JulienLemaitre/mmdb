...
function mkItem(label: string, id: string): RequiredChecklistItem {
  return { entityType: 'SECTION', entityId: id, fieldPath: `SECTION:${id}:rank`, label }
}

function TestChecklist() {
  const [checked, setChecked] = React.useState<Record<string, boolean>>({})
  const items = React.useMemo(() => [mkItem('First', 's-1'), mkItem('Second', 's-2')], [])

  const allChecked = items.every((it) => checked[`${it.entityType}:${it.entityId}:${it.fieldPath}`])
  ...
  return (
    <div>
      ...
      <button type="button" disabled={!allChecked}>Submit review</button>
    </div>
  )
}

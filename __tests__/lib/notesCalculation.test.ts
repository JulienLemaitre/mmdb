import { getNotesPerSecondsFromNoteValues, getNoteValuesFromNotesPerSecond } from '@/lib/notesCalculation'
import {NOTE_VALUE} from "@prisma/client";

describe('getNotesFromNotesPerSecond', () => {
  test('QUARTER = 60 / 1 nps => QUARTER', () => {
    const res = getNoteValuesFromNotesPerSecond({ metronomeMark: { beatUnit: 'QUARTER', bpm: 60, notesPerSecond: { fastestStructuralNote: 1 } } })

    expect(res.fastestStructuralNoteValue).toEqual('QUARTER')
  })
  test('QUARTER = 60 / 2 nps => EIGHTH', () => {
    const res = getNoteValuesFromNotesPerSecond({ metronomeMark: { beatUnit: 'QUARTER', bpm: 60, notesPerSecond: { fastestStructuralNote: 2 } } })

    expect(res.fastestStructuralNoteValue).toEqual('EIGHTH')
  })
  test('QUARTER = 60 / 3 nps => TRIPLET_EIGHTH', () => {
    const res = getNoteValuesFromNotesPerSecond({ metronomeMark: { beatUnit: 'QUARTER', bpm: 60, notesPerSecond: { fastestStructuralNote: 3 } } })

    expect(res.fastestStructuralNoteValue).toEqual('TRIPLET_EIGHTH')
  })
  // 4 Rondos brillants, Op.2, 3 - D Minor, 1 - Andante
  test('QUARTER = 108 / 28.8 nps => SIXTYFOURTH', () => {
    const res = getNoteValuesFromNotesPerSecond({ metronomeMark: { beatUnit: 'QUARTER', bpm: 108, notesPerSecond: { fastestStructuralNote: 28.8 } } })

    expect(res.fastestStructuralNoteValue).toEqual('SIXTYFOURTH')
  })
})
...
describe('getNotesFromNotesPerSecond_function', () => {
  ...
  test("test_missing_notesPerSecond", () => {
    ...
    // @ts-ignore
    expect(() => getNoteValuesFromNotesPerSecond({metronomeMark})).toThrowError("No notesPerSecond property found in given section")
  })
  test("test_missing_fastestNotes", () => {
    ...
    expect(() => getNoteValuesFromNotesPerSecond({metronomeMark})).toThrowError()
  })
  test("test_getNote_called_correctly", () => {
    ...
    getNoteValuesFromNotesPerSecond({metronomeMark, getNoteMock})
  ...
  test("test_getNote_throws_error", () => {
    ...
    expect(getNoteValuesFromNotesPerSecond({metronomeMark, getNoteMock})).toEqual(expectedNotes)
  })

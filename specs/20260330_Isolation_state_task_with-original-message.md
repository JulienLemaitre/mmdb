...
Je me rends compte que j'ai fait une erreur dans la gestion de mes states dans mon app et j'ai besoin de planifier une correction assez importante.

**Voici le contexte**
La feature `feed` est un formulaire complexe à plusieurs étapes et sous-étapes qui permet de renseigner toutes les informations qui contribuent à déterminer la vitesse d'exécution des différentes parties des pièces contenues dans une édition de partitions de musique. La description générale de ces étapes se trouve dans la section help @file:stepHelpContent.tsx

Un state qui couvre la portée de toute l'édition y est géré via le context @file:feedFormContext.tsx

**Le sujet plus précis**
Lorsque l'on décide d'ajouter une pièce simple (single piece), cela ouvre un nouveau sous formulaire @file:SinglePieceVersionFormContainer.tsx qui a son propre state géré par @file:singlePieceVersionFormContext.tsx

On peut y définir en 3 étapes :
1. le compositeur
2. le titre et la date de la pièce
3. La structure de la piece (pieceVersion) : ses différents mouvements et les différentes sections de chaque mouvement.

Lorsque l'on ajoute une nouvelle pièce, le state initial de ce sous formulaire est null.
Lorsque l'on modifie une pièce ajoutée auparavant, un state initial est généré dans la méthode `onEditMMSourceOnPieceVersion` de @file:SourceOnPieceVersionFormContainer.tsx

Une info importante : chaque formulaire est implémenté avec React-Hook-Form, donc les données du formulaire ne sont enregistrées dans le state qu'à la soumission du formulaire de chaque étape, une fois les validations passées. En conséquence, je pense que les entitées dans le state de sous-formulaire `SinglePieceVersionFormContainer` doivent être les mêmes objets complets que dans le state général, pas des brouillons incomplets.

**Le problème**
Les données de la pièce qui est en train d'être ajoutée ou mise à jour sont directement enregistrées dans le state général du `feedForm` .
Seules les id de composer, piece et pieceVersion sont dans le state du sous-formulaire.

**Le besoin**
Toutes les données créées et éditées dans le sous-formulaire de création/mise à jour d'une pièce simple devraient se trouver uniquement dans le state de ce formulaire, et être seulement retranscrit dans le state du formulaire général feedForm qu'à la confirmation finale des nouvelles données à la fin du sous-formulaire.

**Ta tâche**
- Analyser le fonctionnement des deux states en question
- Définit les entités requises et les modifications à apporter au state du sous-formulaire @file:singlePieceVersionFormContext.tsx
- Détermine les modifications à apporter au reducer de ce context.
- Planifie précisément l'ensemble des tâches nécessaires pour mener ce projet à bien.
- Génère un document de spécifications détaillées avec une chronologie de sa mise en œuvre.

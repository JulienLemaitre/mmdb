Below is a pragmatic, future‑proofed review process design that stays simple enough for a solo developer while giving you robust correctness, auditability, and room to grow.
...
- Phase 1 (MVP, low risk)
    - Add Review, ReviewedEntity, AuditLog tables + MMSource.reviewState column.
    - Start/abort endpoints and list filtering.
    - Checklist UI driven by a hard‑coded ReviewChecklistSchema map.
    - Finalize endpoint: transactional apply + audit + denormalized reviewState update.
- Phase 2 (comfort and quality)
    - Add filters/progress indicators, show changed vs unchanged.
    - Add admin panel for:
        - Viewing active reviews and aborting them.
        - Searching AuditLog by entity.
    - Idempotency key for finalize endpoint.
- Phase 3 (optional)
    - Server‑side “draft” storage for long reviews (ReviewDraft table), syncing every X minutes to avoid localStorage reliance.

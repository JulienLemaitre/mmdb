...
export default function PieceVersionUpdate({ searchParams: { pieceVersionId } }) {
  const { state } = useEditForm();
  const [pieceVersion, setPieceVersion] = useState<PieceVersionState | null>(null);
  const [isPieceVersionInitialized, setIsPieceVersionInitialized] = useState(false);

  useEffect(() => {
    const statePieceVersion = state.pieceVersion;

    if (!pieceVersionId) {
      console.log(`[PieceVersionUpdate] pieceVersionId is undefined, get context value`);
      if (!statePieceVersion) {
        console.log(`[PieceVersionUpdate] ERROR: context pieceVersion is undefined`);
  ...
  console.log(`[PieceVersionUpdate] initial pieceVersion value :`, pieceVersion);
  const pieceVersionInput = getPieceVersionInputFromPieceVersionState(pieceVersion);
